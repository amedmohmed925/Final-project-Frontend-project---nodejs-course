...
import { getAllTeachers, activateUser, deactivateUser, changeUserRole, deleteUserByAdmin, advancedUserSearch } from "../api/userAdminApi";
      ...
      <Form onSubmit={handleSearch} className="mb-3 p-3 border rounded bg-light">
        <Row className="align-items-center g-2">
          ...
          <Col xs={12} md={12} className="mt-2">
            ...
            <Button variant="secondary" onClick={fetchTeachers} disabled={loading || searching}>Reset</Button>
          </Col>
        </Row>
      </Form>
          ...
          <tbody>
            {teachers.map((t, i) => (
              <tr key={t._id}>
                ...
                <td>{t.email}</td>
                <td>{t.isVerified ? "Yes" : "No"}</td>
                <td>
                  <Form.Select
                    size="sm"
                    value={t.role}
                    onChange={e => handleRoleChange(t, e.target.value)}
                    disabled={selected===t._id}
                  >
                    <option value="teacher">Teacher</option>
      ...
      <Modal show={roleModal.show} onHide={() => setRoleModal({ show: false, user: null, newRole: "" })} centered backdrop={false}>
        ...
        <Modal.Footer>
          ...
          <Button variant="danger" onClick={() => doChangeUserRole(roleModal.user, roleModal.newRole)} disabled={selected===roleModal.user?._id}>Confirm</Button>
                ...
                <td>
                  {t.isVerified ? (
                    <Button size="sm" variant="warning" disabled={selected===t._id} onClick={() => handleAction(t, "deactivate")}>Deactivate</Button>
                  ) : (
                    <Button size="sm" variant="success" disabled={selected===t._id} onClick={() => handleAction(t, "activate")}>Activate</Button>
                  )}
                  <Button size="sm" variant="danger" className="ms-2" disabled={selected===t._id} onClick={() => setModal({ show: true, user: t, action: "delete" })}>Delete</Button>
                </td>
              </tr>
            ))}
          </tbody>
